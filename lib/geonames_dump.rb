  ...
  def self.search(query, options = {})
    ...
    begin
      ...
      when :auto # return an array of features
        ...
        # city name
        cities = GeonamesCity.joins("left join geonames_countries on country_code = iso").where("to_tsvector(country||' '||name||' '||asciiname||' '||alternatenames) @@ plainto_tsquery('#{query}')").select("geonames_features.*,geonames_countries.country as country, 'city' as tag_type").limit(options[:limit])
